...
class PaymentClaimAccountVisibilityInstrumentedTest {
    ...
    @Test fun payableAccountShowsPaymentClaimAction() {
        composeRule.setContent { WaslApp(repository = repository, instanceKey = "payment-claim-payable-ui", requestedDebtId = "payable-claim-ui") }
        waitForTag("account-remaining")
        scrollToTag("add-payment-claim")
        composeRule.onNodeWithTag("add-payment-claim").assertIsDisplayed()
    }

    @Test fun receivableAccountDoesNotShowPaymentClaimAction() {
        composeRule.setContent { WaslApp(repository = repository, instanceKey = "payment-claim-receivable-ui", requestedDebtId = "receivable-no-claim-ui") }
        waitForTag("account-remaining")
    ...
    private suspend fun createDebt(id: String, personName: String, direction: DebtDirection) {
        repository.createPersonWithDebt(CreatePersonWithDebtCommand(personId = PersonId("person-$id"), debtId = DebtId(id), personName = personName, direction = direction, originalAmount = Money(50_000L, CurrencyCode.YER), openedAt = Instant.parse("2026-08-20T08:00:00Z"), createdAt = Instant.parse("2026-08-20T08:00:00Z")))
    }
