...
## 2026-09-17 — أصول الشعار والتحقق قبل تطبيع المبالغ

- مصدر الحروف هو التصميم المرئي الذي اعتمده المالك، لا الرسم التقريبي السابق. تم تتبع حدود الحروف من المرجع النقطي، وإعادة بناء القوس والنقطتين هندسيًا. هذه ليست نسخة أصلية عالية الدقة من المصمم، ولا تحويلًا لخط غير موجود في المستودع.
- موارد VectorDrawable ثابتة دون خط أو Bitmap وقت التشغيل. تطابق الهندسة بين اللون والأحادي ودائرة الأمان محمي باختبارات المصدر.
- تطبيع فواصل المال يأتي بعد التحقق من مواضعها، ولا يقبل BigDecimal مباشرةً نصوص الأسس أو مدخلات غير محدودة. لا يتغير تمثيل المال أو Schema.
- المحلل الطبيعي يعيد استخدام MoneyInputParser بدل تحويل Long مستقل يقتطع الكسور ويتجاوز الحدود. الحالات الغامضة لا تنتج رقمًا تخمينيًا.
- تبقى CI الحالية بدون محاكي أو cache دائم كما تقرر في سبتمبر. نجاح اختبارات المصدر ليس ادعاءً بتجربة الجهاز أو سلامة كل مسار في التطبيق.
